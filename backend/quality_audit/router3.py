...
async def get_dropdowns(current_user: str = Depends(get_current_user)):
    ...
    pnd_set = set()
    for doc in employee_details_collection.find({"Grade Name": {"$exists": True, "$ne": ""}}, {"PnD": 1, "_id": 0}):
        v = (doc.get("PartnerEmpName") or "").strip()
        if v: pnd_set.add(v)
    if not pnd_set:
        for doc in employee_details_collection.find({"PartnerEmpName": {"$exists": True, "$ne": ""}}, {"PartnerEmpName": 1, "_id": 0}):
            v = (doc.get("PartnerEmpName") or "").strip()
            if v: pnd_set.add(v)
...
async def dashboard_stats(current_user: str = Depends(get_current_user)):
    ...
    records = []
    tl_scores:    dict[str, list] = {}
    pnd_scores:   dict[str, list] = {}
    client_scores:dict[str, list] = {}
    month_counts: dict[str, int]  = {}
    section_scores: dict[str, dict] = {k: {"score": 0, "ideal": 0, "count": 0} for k in SECTION_LABELS}
    particular_scores: dict[str, dict] = {}
    misses_list = []          # items where misses/serious field is filled
    ...
    for doc in all_docs:
        ...
        sc   = float(doc.get("total_score", 0)  or 0)
        isc  = float(doc.get("ideal_score", 0)  or 0)
        sca  = float(doc.get("scaled_total", 0) or 0)

        mon  = sat.strftime("%b %Y") if isinstance(sat, datetime) else "Unknown"
        month_counts[mon] = month_counts.get(mon, 0) + 1

        if tl:
            tl_scores.setdefault(tl, []).append(sca)
        if pnd:
            pnd_scores.setdefault(pnd, []).append(sca)
        if cli:
            client_scores.setdefault(cli, []).append(sca)

        # Section-level breakdown
        for sec_key, sec_label in SECTION_LABELS.items():
            items = doc.get(sec_key, [])
            for it in items:
                s  = float(it.get("score", 0) or 0)
                id_ = float(it.get("ideal_score", 0) or 0)
                ...
                if p:
                    particular_scores.setdefault(p, {"score": 0, "ideal": 0, "count": 0})
                ...
                if ms:
                    misses_list.append({
                        ...
                        "tl":         tl,
                        "client":     cli,
                        "date":       doc.get("audit_date") or (sat.strftime("%d %b %Y") if isinstance(sat, datetime) else ""),
                    })
        ...
        records.append({
            ...
            "project_pnd":  pnd,
            "client_name":  cli,
            "submitted_at": sat.isoformat() if isinstance(sat, datetime) else "",
            "total_score":  sc,
            "ideal_score":  isc,
            "scaled_total": sca,
            "submitted_by": doc.get("submitted_by", ""),
    ...
    # Month trend
    month_trend = [{"month": k, "count": v} for k, v in sorted(month_counts.items())]
    ...
    for sec_key, sec_label in SECTION_LABELS.items():
        ...
        section_breakdown.append({"section": sec_label, "score": sd["score"], "ideal": sd["ideal"], "pct": pct})

    # Particular breakdown (item-wise) — sort by lowest pct (areas needing attention)
    particular_breakdown = []
    for p, pd in particular_scores.items():
        pct = round(pd["score"] / pd["ideal"] * 100, 1) if pd["ideal"] > 0 else 0
        particular_breakdown.append({"particular": p[:80], "score": pd["score"], "ideal": pd["ideal"], "pct": pct})
    particular_breakdown.sort(key=lambda x: x["pct"])

    # Top & Bottom 5 clients
